...
1. 긍정적 요소 강화: 이미 좋게 작동하는 경험, 만족, 기대, 몰입 요소를 더 크게 키우는 질문
2. 부정적 요소 제거: 반복되는 불편, 마찰, 불안, 혼란을 덜어내거나 사라지게 하는 질문
3. 반대로 뒤집기: 사용자가 찾아오게 하는 대신 찾아가는 방식처럼 문제 상황을 반전하는 질문
4. 가정에 의문 제기: 반드시 필요하다고 믿는 절차, 장소, 순서, 역할, 규칙을 다시 묻는 질문
5. 형용사 사용: 쉬운, 안전한, 즐거운, 자신감 있는, 몰입되는 등 사용자가 기억할 감정으로 바꾸는 질문
6. 예상치 못한 자원 활용: 기존 콘텐츠, 동료, 강사 피드백, 학습 기록, 템플릿, 데이터 등 숨어 있는 자원을 활용하는 질문
7. 유추와 비유: 다른 산업/상황의 경험을 빌려 새로운 해결 방향을 열어주는 질문
...
- 최종 3개 HMW는 서로 다른 기회 렌즈를 적용해 중복되지 않게 작성하세요.
"""
    ...
    try:
        ...
        if st.button("대시보드 생성하기 (Pain Point & HMW 포함)"):
            with st.spinner('AI가 데이터를 분석하여 6단계 대시보드를 생성 중입니다. 다소 시간이 걸릴 수 있습니다...'):
                ...
                6. HMW (How Might We) 도출: 재정의된 문제를 바탕으로, 아래 [HMW 기회 요소 발견 가이드]를 적용하여 긍정적이고 창의적인 해결책을 촉발할 수 있는 '우리가 어떻게 하면 ~할 수 있을까?' 질문 3가지 작성
                   - 각 HMW는 서로 다른 기회 렌즈를 적용하고, h4에는 적용한 기회 렌즈와 해결 방향을 함께 표시하세요.
                   - p에는 사용자, 의도하는 행동/경험, 원하는 결과가 드러나는 완성형 질문을 작성하세요.
                   - 질문 안에 특정 솔루션을 미리 넣지 말고, 아이디어가 다양하게 나올 수 있는 여지를 남기세요.
                        ...
                        <div class="hmw-list">
                            <div class="hmw-item">
                                ...
                                <div class="hmw-content">
                                    <h4>기회 렌즈: 여기에 적용한 기회 요소와 해결 방향성(예: 형용사 사용 · 막막한 기획을 자신감 있는 첫걸음으로 전환)을 작성하세요</h4>
                                    <p>우리가 어떻게 하면 [사용자]가 [의도하는 행동/경험]을 통해 [원하는 결과]를 얻을 수 있을까?</p>
                                </div>
                            </div>
                            <div class="hmw-item">
                                <div class="hmw-icon">🚀</div>
                                <div class="hmw-content">
                                    <h4>기회 렌즈: 여기에 두 번째 기회 요소와 해결 방향성을 작성하세요</h4>
                                    <p>우리가 어떻게 하면 [사용자]가 [의도하는 행동/경험]을 통해 [원하는 결과]를 얻을 수 있을까?</p>
                                </div>
                            </div>
                            <div class="hmw-item">
                                <div class="hmw-icon">🏆</div>
                                <div class="hmw-content">
                                    <h4>기회 렌즈: 여기에 세 번째 기회 요소와 해결 방향성을 작성하세요</h4>
                                    <p>우리가 어떻게 하면 [사용자]가 [의도하는 행동/경험]을 통해 [원하는 결과]를 얻을 수 있을까?</p>
                                </div>
                            </div>
                        </div>
